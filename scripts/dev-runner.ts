...
function resolveBaseDir(baseDir: string | undefined): Effect.Effect<string, never, Path.Path> {
  return Effect.gen(function* () {
    ...
    return yield* DEFAULT_KATACODE_HOME;
  });
}
...
interface CreateDevRunnerEnvInput {
  ...
  readonly katacodeHome: string | undefined;
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const resolvedBaseDir = yield* resolveBaseDir(katacodeHome);
    ...
    if (!isDesktopMode && host !== undefined) {
      output.KATACODE_HOST = host;
    }

    if (!isDesktopMode && noBrowser !== undefined) {
      output.KATACODE_NO_BROWSER = noBrowser ? "1" : "0";
    } else if (!isDesktopMode) {
      delete output.KATACODE_NO_BROWSER;
    }

    if (autoBootstrapProjectFromCwd !== undefined) {
      output.KATACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.KATACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.KATACODE_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.KATACODE_LOG_WS_EVENTS;
    }

    if (mode === "dev") {
      output.KATACODE_MODE = "web";
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read KATACODE_PORT_OFFSET/KATACODE_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const env = yield* createDevRunnerEnv({
      ...
      serverOffset,
      webOffset,
      katacodeHome: input.katacodeHome,
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.KATACODE_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.KATACODE_HOME)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to KATACODE_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("KATACODE_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to KATACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("KATACODE_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to KATACODE_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("KATACODE_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to KATACODE_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("KATACODE_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to KATACODE_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("KATACODE_PORT")),
  ),
