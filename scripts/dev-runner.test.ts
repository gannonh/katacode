  ...
  describe("resolveOffset", () => {
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid KATACODE_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults KATACODE_HOME to ~/.katacode when not provided", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.KATACODE_HOME, path.resolve(NodeOS.homedir(), ".katacode"));
      }),
    );
    ...
    it.effect("uses custom katacodeHome when provided", () =>
      Effect.gen(function* () {
        const path = yield* Path.Path;
        const env = yield* createDevRunnerEnv({
          mode: "dev",
