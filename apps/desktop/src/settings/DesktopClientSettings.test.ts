...
import { ClientSettingsSchema, type ClientSettings } from "@kata-sh/code-contracts";
...
function makeLayer(baseDir: string) {
  const environmentLayer = DesktopEnvironment.layer({
    ...
    Layer.provide(
      Layer.mergeAll(NodeServices.layer, DesktopConfig.layerTest({ KATACODE_HOME: baseDir })),
    ),
  );
