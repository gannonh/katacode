...
function makeEnvironmentLayer(baseDir: string, env: Record<string, string | undefined> = {}) {
  return makeDesktopEnvironmentLayer({
    ...
    Layer.provide(
      Layer.mergeAll(
        NodeServices.layer,
        DesktopConfig.layerTest({ KATACODE_HOME: baseDir, ...env }),
      ),
    ),
  );
}
...
function makeLayer(input: {
  ...
  const env = { KATACODE_HOME: input.baseDir, ...input.env };
...
describe("DesktopServerExposure", () => {
  ...
  it.effect("uses ConfigProvider desktop exposure overrides", () =>
    withHarness(
      ...
      {
        KATACODE_DESKTOP_LAN_HOST: "10.0.0.7",
        KATACODE_DESKTOP_HTTPS_ENDPOINTS: "https://public.example.test",
      },
    ),
  );
