...
describe("Pierre file icons", () => {
  ...
  it("extends Pierre with KataCode-specific exact filename icons", () => {
    ...
    assert.equal(
      ...
      "katacode-file-icon-tsconfig",
    );
    assert.equal(resolvePierreIconForEntry("AGENTS.md", "file")?.name, "katacode-file-icon-agents");
    assert.equal(resolvePierreIconForEntry("CLAUDE.md", "file")?.name, "katacode-file-icon-claude");
    assert.equal(resolvePierreIconForEntry("README.md", "file")?.name, "katacode-file-icon-readme");
    assert.equal(
      resolvePierreIconForEntry("pnpm-lock.yaml", "file")?.name,
      "katacode-file-icon-pnpm",
    );
    assert.equal(
  ...
  it("ships every custom icon referenced by the extended resolver", () => {
    const customIconNames = new Set(Object.values(KATACODE_PIERRE_ICONS.byFileName));
    for (const iconName of customIconNames) {
      assert.include(KATACODE_PIERRE_ICONS.spriteSheet, `id="${iconName}"`);
    }
  });
