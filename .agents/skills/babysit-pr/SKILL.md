...
```bash
python3 <path-to-agent-skills-directory>/babysit-pr/scripts/gh_pr_watch.py --pr auto --once
```

### Continuous watch (JSONL)

```bash
python3 <path-to-agent-skills-directory>/babysit-pr/scripts/gh_pr_watch.py --pr auto --watch
```

### Trigger flaky retry cycle (only when watcher indicates)

```bash
python3 <path-to-agent-skills-directory>/babysit-pr/scripts/gh_pr_watch.py --pr auto --retry-failed-now
```
...
- Heuristics and decision tree: `<path-to-agent-skills-directory>/babysit-pr/references/heuristics.md`
- GitHub CLI/API details used by the watcher: `<path-to-agent-skills-directory>/babysit-pr/references/github-api-notes.md`
