...
if (!evidenceDir || !screenshotPath) {
  console.error(
    "Usage: cdp-capture-page.mjs --evidence <dir> --cdp http://127.0.0.1:9222 [--title <title>|--url-contains <text>] --screenshot <path> [--text <path>]",
  );
  process.exit(2);
}
...
if (existsSync(join(evidenceDir, "evidence.json"))) {
  ...
  manifest.artifacts.push({
    ...
    description: `CDP screenshot for ${target.title}`,
  });
  if (resolvedTextPath)
    manifest.artifacts.push({
      type: "log",
      path: resolvedTextPath,
      description: `CDP text snapshot for ${target.title}`,
    });
