...
describe("ssh command", () => {
  ...
  it.effect("resolves the remote CLI package spec from the desktop release channel", () =>
    Effect.sync(() => {
      assert.equal(
        resolveRemoteCliPackageSpec({
          appVersion: "0.0.17",
      ...
      assert.equal(
        ...
        "@kata-sh/code-cli@0.0.17-nightly.20260415.44",
      );
      assert.equal(
        resolveRemoteCliPackageSpec({
