...
describe("ssh auth", () => {
  ...
  it.effect("creates askpass env for cached password prompts", () =>
    Effect.gen(function* () {
      ...
      assert.equal(env.KATACODE_SSH_AUTH_SECRET, "super-secret");
      assert.equal(env.DISPLAY, "katacode");
  ...
  it.effect("builds a windows askpass launcher pair", () =>
    Effect.gen(function* () {
      const descriptor = yield* buildSshAskpassHelperDescriptor({
        directory: "C:\\temp\\katacode-ssh-askpass",
      }).pipe(
        Effect.provide(
      ...
      assert.equal(descriptor.launcherPath, "C:\\temp\\katacode-ssh-askpass\\ssh-askpass.cmd");
      assert.deepEqual(
