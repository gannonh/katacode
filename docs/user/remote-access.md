...
with the same server-side behavior as `katacode serve --tailscale-serve`, then the server asks Tailscale
...
The server logs a warning on startup when it detects `~/.t3` but not `~/.katacode`.
