...
- Reads the shared production KataCode Connect relay URL and Clerk client configuration before packaging clients.
...
## KataCode Connect relay deployment
...
- `KATACODE_WEB_ROUTER_URL`: defaults to `https://app.t3.codes`.
- `KATACODE_WEB_LATEST_DOMAIN`: defaults to `latest.app.t3.codes`.
- `KATACODE_WEB_NIGHTLY_DOMAIN`: defaults to `nightly.app.t3.codes`.
...
- Temporary private-repo auth workaround:
  - set `KATACODE_DESKTOP_UPDATE_GITHUB_TOKEN` (or `GH_TOKEN`) in the desktop app runtime environment.
