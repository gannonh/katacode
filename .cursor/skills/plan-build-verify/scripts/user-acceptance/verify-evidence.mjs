...
if (manifest) {
  ...
  if (uiTarget) {
    const hasVisible = (manifest.artifacts ?? []).some((artifact) =>
      ["video", "screenshot"].includes(artifact.type),
    );
    const hasVideoSkipped = (manifest.notes ?? []).some((note) =>
      /video.*skipp|screenshot/i.test(note),
    );
    if (!hasVisible && !hasVideoSkipped)
      fail(
        "UI target needs a video/screenshot artifact or an explicit note explaining why visual capture was skipped",
      );
  }
}
